...
class WPUPllUtilities {
    ...
    public function glob_recursive($base, $pattern, $flags = 0) {
        ...
        foreach (glob($base . '*', GLOB_ONLYDIR | GLOB_MARK) as $dir) {
            $dirname = basename($dir);
            if (strpos($dirname, 'node_modules') !== false || strpos($dirname, '.git') !== false || strpos($dirname, 'assets') !== false) {
                continue;
            }
    ...
    public function admin_menu() {
        if (!current_user_can('manage_options') && current_user_can($this->user_level) && function_exists('PLL')) {
    ...
    public function admin_footer_load_translation_js() {
...
add_action('wp_enqueue_scripts', function () {
    $languages = wpu_pll_utilities_get_languages();
